            ...
            <div class="card mt-2 mx-auto p-4 bg-light">
                <div class="card-body bg-light">
                    ...
                    <form @submit.prevent="onSubmit">
                            ...
                            <br />
                            <div class="form-group">
                            <button class="btn btn-primary btn-block btn-lg">Add Category</button>
                            </div>
                    </form>
                        </div>
                        <div v-else>
                         <h4>{{message}}</h4>
                        <button class="btn btn-success" @click="newCategory">Add Category</button>
                        </div>
                </div>
            </div> 
        </div> 
...
export default {
    ...
    methods: {
        ...
        formData.append('avt' , this.FILE , this.FILE.name)
        var object = {};
        object['name'] = this.name
        object['description'] = this.description
        var json = JSON.stringify(object);
        formData.append('jsonObject' , json)
        axios.post('http://localhost:8080/api/v1/category/add' , formData , { headers : {
